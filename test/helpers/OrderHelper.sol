...
abstract contract OrderHelper is Test {
    ...
    function _initOrderHelper(bytes32 _domainSeparator, address _defaultCollection, address _defaultCurrency) internal {
    ...
    function makeAsk(address collection, address currency, address actor)
        internal
        view
        returns (OrderModel.Order memory)
    {
    ...
    function makeOrder(OrderModel.Side side, bool isCollectionBid, address actor)
        internal
        view
        returns (OrderModel.Order memory)
    {
        return _order(side, isCollectionBid, defaultCollection, defaultCurrency, DEFAULT_PRICE, actor, 0);
    }
    ...
    function makeSignedAsk(address signer, uint256 signerPk)
        internal
        view
    ...
    function signOrder(OrderModel.Order memory order, uint256 signerPk)
        internal
        view
        returns (bytes32 digest, SigOps.Signature memory sig)
    {
